...
def first_pass_yield(days: int = 7) -> float:
    ...
    qs = Inspection.objects.filter(attempt_number=1, completed_at__isnull=False, started_at__gte=since)
    denom = qs.count()
    if denom == 0:
        return 0.0
    num = qs.filter(final_result="PASS").count()
    return round((num / denom) * 100.0, 2)


def avg_qc_time_hours(days: int = 7) -> float:
    since = timezone.now() - timedelta(days=days)
    qs = Inspection.objects.filter(completed_at__isnull=False, started_at__gte=since).only("started_at", "completed_at")
...
def urgent_sla_breaches(hours_threshold: int = 6) -> int:
    ...
    return Unit.objects.filter(
        priority="URGENT",
        status__in=["RECEIVED", "QC_IN_PROGRESS", "REWORK", "RETEST"],
...
def auto_flag(defect_threshold_percent: float = 10.0, days: int = 7, min_sample: int = 10) -> int:
    ...
    for row in lab_totals:
        ...
        if rate >= defect_threshold_percent:
            _, was_created = QualityFlag.objects.get_or_create(
                flag_type="LAB",
                flag_key=lab,
